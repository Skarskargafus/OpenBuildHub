...
(function() {
	...
	document.addEventListener('DOMContentLoaded', () => {
		...
		function getHeaderOffset() {
			if (window.innerWidth < 768) {
				return '0%'; // Don't move header on mobile
		...
		const tl = gsap.timeline({
			scrollTrigger: {
				trigger: section,
				start: 'top top',
				end: `+=${cards.length * 120}%`, // Reduced from 120% to 100%
				scrub: 0.5,
				pin: true,
				anticipatePin: 1,
				// onLeave: (self) => {
				// 	self.disable(); // Disable the ScrollTrigger when leaving the section
				// 	setTimeout(() => self.enable(), 1000); // Re-enable after a short delay
		...
		cards.forEach((card, index) => {
			...
			if (isMobile()) {
				tl.to(card.querySelector('.card-header'), {
					y: '0%', // Don't move the header on mobile
					duration: 0.3
				}, '<');
			} else {
				tl.to(card.querySelector('.card-header'), {
	...
	function showNotification(content) {
		...
		if (window.innerWidth <= 768) {
		  // On mobile, replace the existing notification
		  notificationContent.innerHTML = content;
		} else {
		  // On desktop, append the new notification
		  notificationContent.innerHTML += content;
		}
	  }
